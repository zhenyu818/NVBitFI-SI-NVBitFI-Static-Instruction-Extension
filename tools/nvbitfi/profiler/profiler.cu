...
void instrument_function_if_needed(CUcontext ctx, CUfunction func) {
	...
	for (auto f : related_functions) {
		...
		for (auto i : instrs) {
            ...
            // assert(fout_log.good());
			// fout_log<<getOpGroupNum(instTypeNameMap[instType])<<' '<<i->getSass()<<"\n";
            // std::map<int, std::vector<long>> static_id_to_dynamic_id_map;
            ...
            nvbit_add_call_arg_const_val64(i, (uint64_t)static_inst_threads_count);
            nvbit_add_call_arg_const_val32(i, i->getIdx());
            nvbit_add_call_arg_const_val64(i, (uint64_t)static_inst_type);
			if (verbose==2) { 
				printf("end..\n");
			}
		}
...
void nvbit_at_cuda_event(CUcontext ctx, int is_exit, nvbit_api_cuda_t cbid, const char * name, void * params, CUresult * pStatus) {
	if (cbid == API_CUDA_cuLaunch ||
		...
		if(!is_exit) {
			...
			assert(fout_log.good());
            assert(fout_map.good());
			std::string kname = removeSpaces(nvbit_get_func_name(ctx, p->f)).c_str();
			// std::cout << "; kernel_name: " << kname << " " << nvbit_get_func_name(ctx, p->f) << "\n";
			fout_log << "NVBit-igprofile; index: " << kernel_id++ << "; kernel_name: " << kname 
				<< "; ctas: " << num_ctas;
			if (enable_instrumentation) {
				fout_log << "; instrs: " << get_inst_count(true) << ";" ;
                const std::vector<Instr *> &instrs = nvbit_get_instrs(ctx, p->f);
				for (int i=0; i<NUM_COUNTERS; i++) {
                    // if(i==NUM_ISA_INSTRUCTIONS)
                    //     fout_log<<"\n"<<"NUM_ISA_INSTRUCTIONS"<<"\n";
					if (i < NUM_ISA_INSTRUCTIONS) {
						fout_log << " " << instTypeNames[i] << ": " << counters[i] << ",";
					} else {
						fout_log << " " << instGrouptNames[i-NUM_ISA_INSTRUCTIONS] << ": " << counters[i] << ",";
					}
				}
                for (int i = 0; i < instrs.size(); i++) {
                    fout_map<< kname <<" "<< kernel_id-1 << " " << i << " " << static_inst_type[i] << " " << static_inst_threads_count[i]<< " \"" <<instrs[i]->getSass() << "\"" << "\n";
                }
			} else {
				int pos = line_buffer.find("; instrs: "); 
				fout_log << line_buffer.substr(pos); // simply record the previously obtained counts
			}
			fout_log << "\n";
			fout_log.flush();
            fout_map.flush();
