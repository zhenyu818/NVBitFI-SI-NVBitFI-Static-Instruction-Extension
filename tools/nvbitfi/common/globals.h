...
std::string static_to_dynamic_map = "static-to-dynamic-map-temp.txt";
std::ofstream fout_log;
std::ofstream fout_map;
std::ofstream fout;
...
void open_output_file()
{
    ...
    if (!open_flag)
    {
        fout_log.open(injOutputFilename.c_str(), std::ifstream::out);
        fout_map.open(static_to_dynamic_map.c_str(), std::ifstream::out);
        open_flag = true;
    }
}
void open_output_file_fout(std::string filename)
{
    static bool open_flag = false; // file will be parsed only once - performance enhancement
    if (!open_flag)
    {
...
__managed__ uint64_t static_inst_threads_count[NUM_STATIC_INSTS];
__managed__ uint64_t static_inst_type[NUM_STATIC_INSTS];
__managed__ uint64_t threadCounter[2]; // instruction counters

uint64_t get_inst_count(bool profiler = false)
{
    uint64_t sum2 = 0;
    for (int i = NUM_ISA_INSTRUCTIONS; i < NUM_COUNTERS - 2; i++)
    { // TODO: 2 here is for G_GPPR and G_GP
        sum2 += counters[i];
    }
    if (profiler)
    {
...
void init_counters()
{
    ...
    for (int i = 0; i < NUM_STATIC_INSTS; i++)
    {
        static_inst_threads_count[i] = 0;
        static_inst_type[i] = 100;
    }
    threadCounter[0] = 0;
    threadCounter[1] = 0;
}
...
std::string extractOpcode(std::string instr)
{
    ...
    if (words.size() > 0)
    {
        ...
        if (words[0][0] == '@')
        { // this is a predicated instruction, 2nd word will be the opcode
            return words[1];
        }
        else
        {
...
int extractRegNo(std::string token, int &num)
{
    ...
    if (numStr.compare("Z") == 0)
    {              // found RZ register
        num = 255; // RZ = R255
    }
    else if (numStr.compare("T") == 0)
    { // found RZ register
        num = -1;
    }
    else
