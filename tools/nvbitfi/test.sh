...
# # make golden
# cd $CWD
cd test-apps/cuda-samples-11.5/Samples/cudaTensorCoreGemm
make clean
make 2> stderr.txt
printf "\nRunning the application without instrumentation\n"
sudo ./cudaTensorCoreGemm > golden_stdout.txt 2>golden_stderr.txt
cd $CWD
...
################################################
printf "\nStep 2: Run the error injection campaign"
python3 run_injections.py standalone # to run the injection campaign on a single machine with single gpu
