...
void reset_inj_info() {
	...
	inj_info.threadID = 0; // instruction id 
...
void parse_params(std::string filename) {
	...
	if (!parse_flag) {
		...
		if (ifs.is_open()) {
			...
			ifs >> inj_info.threadID; // thread id
...
void nvbit_at_init() {
	...
	open_output_file_fout(injOutputFilename);
	if (verbose) 
...
void instrument_function_if_needed(CUcontext ctx, CUfunction func) {
	...
	for (auto f : related_functions) {
		...
		if(strcmp(inj_info.kernelName, kname.c_str()) == 0) { // this is the kernel selected for injection 
			...
			for (auto i : instrs) {
                if(inj_info.instID!=i->getIdx())
                    continue;
				fout << "instID: " << inj_info.instID << "threadID: "<< inj_info.threadID << std::endl;
				...
				nvbit_add_call_arg_const_val64(i, (uint64_t)threadCounter);
