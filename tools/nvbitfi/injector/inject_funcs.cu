...
extern "C" __device__ __noinline__ void inject_error(uint64_t piinfo, uint64_t pthreadCounter, uint64_t pverbose_device, 
	...
	uint32_t verbose_device = *((uint32_t *)pverbose_device);
 	uint64_t * threadCounter = (uint64_t *)pthreadCounter;
	...
	if (predicate == 0)
 	...
 	unsigned long long currCounter = atomicAdd((unsigned long long *)&threadCounter[0], 1);
    ...
    if(inj_info->threadID == currCounter) {
        injectFlag = true;
    }
  
   	if (verbose_device && injectFlag) 
		printf("inj_info->instID=%ld, %ld\n", inj_info->instID, currCounter);
